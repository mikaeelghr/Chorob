...
class MeghdaditSpider(BaseScrapper):
    ...
    def _get_image_url(self, item):
        return 'https://' + self.shop_domain + item.css('img::attr(src)').extract()[0]

    def _get_url(self, item):
        return 'https://' + self.shop_domain + item.css('.d-block::attr(href)').extract()[0]
